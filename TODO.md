...
- [ ] **`json.loads(tc["arguments"])` sin proteger** (`backend/agent.py:499`). Si el modelo genera JSON inválido (muy habitual con llama3.1), explota toda la respuesta con un error genérico. Envolver en try/except y devolver el error al modelo como tool_result para que se autocorrija (igual que ya se hace con el SQL).
