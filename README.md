...
That call runs **after** `fin_respuesta`, once the text has been fully emitted. The TTS does not wait for that event: it starts with the first sentence. So the user hears the answer immediately and the visual appears while they are still listening, preceded by a "preparing" indicator.
